...
class ContextRecognition:
    ...
    def load_model(self):
        with open('/Users/arivan123/Downloads/Emobot/models/tfidf_matrix.pkl', 'rb') as f:
            u = pickle._Unpickler(f)
            u.encoding = 'latin1'
            p = u.load()
            self.tf_idf_matrix = p
            self.model_loaded = True

    def save_model(self):
        with open(self.save_file, 'wb') as handle:
            pickle.dump(self.tf_idf, handle)
        with open('models/tfidf_matrix.pkl', 'wb') as handle:
    ...
    def compute_document_similarity(self, input_sentence):
        ...
        if not self.model_loaded:
            self.tf_idf_matrix = self.tf_idf.fit_transform(list(token_dict.values()))
